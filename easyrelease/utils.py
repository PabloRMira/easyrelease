...
__all__ = ['check_git_dir', 'check_project_root', 'read_credentials', 'get_config', 'get_template', 'update_gitignore',
           'write_settings_ini', 'write_setup_py', 'write_conda_build_scripts']
...
def write_conda_build_scripts():
    ...
    for temp_name in ("bld.bat", "build.sh"):
        if os.path.exists(os.path.join("conda-recipe", temp_name)):
            print(f"{temp_name} already exists. Skipping this step")
        else:
            template = get_template(temp_name)
